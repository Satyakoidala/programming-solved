# Leetcode Problem 1537 ~ Get the Maximum Sum
...
def maxSum1(nums1, nums2):
    ...
    while i<l1 and j<l2:
        if nums1[i]<nums2[j]:
            ...
            if (j+1) < l2:
                graph[nums2[j]].append(nums2[j+1])
            else:
                graph[nums2[j]]=[]
            j+=1
        else:
    ...
    #print(graph)
    if nums1[0]==nums2[0]:
        return maxSumPath(graph,min(nums1[0],nums2[0])) % 1000000007
    return max(maxSumPath(graph,min(nums1[0],nums2[0])),maxSumPath(graph,max(nums1[0],nums2[0]))) % 1000000007

def maxSumPath(self,path,curr_node) -> int:
    if (path[curr_node]==[]): return curr_node
    max_sum = 0
    curr_sum = curr_node
    for node in path[curr_node]:
        max_sum = max(max_sum, self.maxSumPath(path, node))
